<x-layout>
    ...
    <x-admin.section-title title="Register Users" />
        ...
        @foreach ($users as $user)
            <div class="flex flex-1 basis-full p-4 rounded-md border border-secondary-grey gap-8 items-center">
                <div class="flex-1 max-w-[70px]">
                    <img class="h-full w-fit" src="{{ Vite::asset('resources/icons/registered-white.svg') }}" alt="">
                </div>
                <div class="flex-1">
                    <h1 class="text-2xl min-w-max">{{ $user->first_name }}
                        {{ $user->last_name }}</h1>
                </div>
