    ...
    <x-customer.ticket-layout>
        ...
        <section class="flex gap-14">
            <div class="basis-full flex justify-center flex-col gap-4">
                ...
                <x-button.buttons tag="a" class="w-full mt-10" href="/">Go Back to Home</x-button.buttons>
            </div>

            <div class="basis-full flex-col gap-8 flex">
                <div class="border border-secondary-grey rounded-2xl px-8 py-12 relative">
                    <h2 class="font-bold text-[40px]">₱{{ $ticketInfo['total-cost'] }}.00</h2>
                ...
                <div class="flex flex-col gap-4 border border-secondary-grey rounded-2xl px-8 py-12 relative">
                    ...
                    <x-customer.success-label title="Movie Title" label="{{ $ticketInfo['title'] }}" />
                    <x-customer.success-label title="Screen Location" label="Cinema {{ $ticketInfo['cinema-number'] }}" />
                    <x-customer.success-label title="Seats Booked" label="{{ $ticketInfo['quantity'] }} Ticket/s" />
                    <x-customer.success-label title="Time Slot" label="{{ $ticketInfo['time-slot'] }} P.M." />
                    <x-customer.success-label title="Ticket Cost" label="₱{{ $ticketInfo['ticket-cost'] }}.00" />
